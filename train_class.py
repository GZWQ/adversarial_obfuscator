...
class ae_gan():
    def __init__(self):
        ...
        self.attack.compile(loss=['binary_crossentropy','binary_crossentropy'],
                                   optimizer=optimizer,
                                   metrics=['accuracy'])
        ...
        valid,class_ = self.attack(generated_img)

        self.combined_model = Model(input_image,[valid,generated_img,class_])
        self.combined_model.compile(loss=['binary_crossentropy','mae','binary_crossentropy'],
    ...
    def discriminator(self):
        ...
        h = Flatten()(h)
        output_secret = Dense(1, activation='sigmoid')(h)
        output_class = Dense(1,activation='sigmoid')(h)
        discriminator = Model(input, [output_secret,output_class])
    ...
    def train(self,epochs, batch_size=128, sample_interval=50):
        ...
        for epoch in range(epochs):
            ...
            l1 = self.attack.train_on_batch(image_batch_public,[label_public,label_batch_public])
            l2 = self.attack.train_on_batch(generated_images_public,[label_public,label_batch_public])
            l3 = self.attack.train_on_batch(image_batch_secret,[label_secret,label_batch_secret])
            l4 = self.attack.train_on_batch(generated_images_secret,[label_secret,label_batch_secret])

            g_loss1 = self.combined_model.train_on_batch(image_batch_public,[label_public,image_batch_public,label_batch_public])
            g_loss2 = self.combined_model.train_on_batch(image_batch_secret,[label_public,image_batch_secret,label_batch_secret])
